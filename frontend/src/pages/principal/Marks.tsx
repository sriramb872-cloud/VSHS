...
import { Mark, AcademicYear } from '../../types';
...
export const PrincipalMarksPage: React.FC = () => {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([marksService.listMarks({ limit: 100 }), examService.listExams(), academicYearsService.listAcademicYears()])
      .then(([markData, examData, ayData]) => {
        setMarks(markData.items);
  ...
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2">
      <select aria-label="Academic Year" value={filters.academicYear} onChange={e => setFilter('academicYear', e.target.value)} className="rounded-lg border border-slate-200 px-3 py-2 text-xs"><option value="">All Academic Years</option>{unique('academic_year_id').map(id => <option key={String(id)} value={String(id)}>{academicYearsById[id]?.name ?? `Year ${id}`}</option>)}</select>
