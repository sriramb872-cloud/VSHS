...
export const PrincipalReportCardsPage: React.FC = () => {
  ...
  useEffect(() => {
    Promise.all([examService.listExams(), reportCardService.listReportCards({ limit: 100 }), academicYearsService.listAcademicYears()])
      .then(([examData, cardData, ayData]) => {
        setExams(examData.items);
        setCards(cardData.items as unknown as ReportCardResponse[]);
  ...
  const academicYearsById = useMemo(() => Object.fromEntries(academicYears.map(ay => [ay.id, ay])), [academicYears]);

  const contexts = useMemo(() => {
    const map = new Map<string, { year: number; section: number; label: string }>();
    exams.forEach(exam => map.set(`${exam.academic_year_id}-${exam.section_id}`, { year: exam.academic_year_id, section: exam.section_id, label: `${exam.grade_name || `Grade ${exam.grade_id}`} - ${exam.section_name || `Section ${exam.section_id}`} · ${academicYearsById[exam.academic_year_id]?.name ?? `Year ${exam.academic_year_id}`}` }));
    return [...map.values()];
  }, [exams, academicYearsById]);
  ...
  return <div className="space-y-4">
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 p-4 shadow-xs grid grid-cols-1 sm:grid-cols-3 gap-2">
      <select aria-label="Academic Year" value={year} onChange={e => { setYear(e.target.value); setSection(''); }} className="rounded-lg border border-slate-200 px-3 py-2 text-xs"><option value="">Select Academic Year</option>{[...new Set(contexts.map(ctx => ctx.year))].map(value => <option key={value} value={value}>{academicYearsById[value]?.name ?? `Year ${value}`}</option>)}</select>
