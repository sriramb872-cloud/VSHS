...
export const Enrollments: React.FC = () => {
  ...
  // Withdraw state
  const [withdrawingEnrollment, setWithdrawingEnrollment] = useState<StudentEnrollment | null>(null);
  ...
  const openEditEnrollment = (e: React.MouseEvent, enrollment: StudentEnrollment) => {
    e.stopPropagation();
    setEditingEnrollment(enrollment);
    const sec = sections.find((s) => s.id === enrollment.section_id);
    const gid = enrollment.grade_id || (sec ? sec.grade_id : '');
    setEditGradeId(gid || '');
    setEditSectionId(enrollment.section_id || '');
    setEditYearId(enrollment.academic_year_id || '');
    setEditRollNumber(enrollment.roll_number || '');
    setEditError(null);
  };

  const handleEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingEnrollment || !editSectionId || !editYearId) return;
    setEditSubmitting(true);
    setEditError(null);
    try {
      const updated = await enrollmentsService.updateEnrollment(editingEnrollment.id, {
        section_id: Number(editSectionId),
        academic_year_id: Number(editYearId),
      ...
      setEditError(err?.response?.data?.detail || 'Failed to update enrollment.');
    } finally {
  ...
  const handleWithdrawConfirm = async () => {
    ...
    await enrollmentsService.deleteEnrollment(withdrawingEnrollment.id);
    setEnrollments((prev) => prev.filter((item) => item.id !== withdrawingEnrollment.id));
  ...
  const renderEditModal = () => {
    ...
    const studentName =
      editingEnrollment.student_name ||
      editingEnrollment.full_name ||
      `Student #${editingEnrollment.student_id}`;
    const editModalSections = editGradeId
      ? sections.filter((s) => s.grade_id === Number(editGradeId))
      : sections;

    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs">
        <div className="bg-white rounded-3xl p-6 w-full max-w-lg shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
          <div className="flex items-center justify-between pb-4 border-b border-slate-100">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Edit / Transfer Enrollment</h2>
              <p className="text-xs text-slate-500">Update section, academic year, or roll number</p>
            </div>
            <button
          ...
          <form onSubmit={handleEditSubmit} className="space-y-4 mt-4">
            <div>
              ...
              <input
                ...
                readOnly
                value={studentName}
                className="w-full h-11 px-3 rounded-xl border border-slate-200 bg-slate-100 text-slate-700 text-sm cursor-not-allowed"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full h-11 px-3 rounded-xl border border-slate-300 bg-white text-slate-900 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                value={editYearId}
                onChange={(e) => setEditYearId(e.target.value ? Number(e.target.value) : '')}
              >
                <option value="">Select academic year...</option>
                {academicYears.map((ay) => (
                  <option key={ay.id} value={ay.id}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  value={editGradeId}
                  onChange={(e) => {
                    setEditGradeId(e.target.value ? Number(e.target.value) : '');
                    setEditSectionId('');
                  }}
                >
                  <option value="">Select grade...</option>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full h-11 px-3 rounded-xl border border-slate-300 bg-white text-slate-900 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none disabled:bg-slate-50 disabled:text-slate-400"
                  value={editSectionId}
                  onChange={(e) => setEditSectionId(e.target.value ? Number(e.target.value) : '')}
                >
                  <option value="">
                    {!editGradeId
                      ? 'Select grade first...'
            ...
            <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
              ...
              <button
                type="submit"
                disabled={editSubmitting || !editYearId || !editSectionId}
                className="h-10 px-5 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white text-xs font-bold shadow-xs transition-all disabled:bg-slate-300 disabled:cursor-not-allowed disabled:transform-none"
              >
      ...
      <div className="space-y-4">
            ...
            {studentsToDisplay.map((st) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0" onClick={e => e.stopPropagation()}>
                  ...
                  <button
                    type="button"
                    onClick={(e) => openEditEnrollment(e, st)}
                    className="p-1.5 rounded-lg border border-slate-200 text-slate-600 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50 transition-all text-xs font-semibold flex items-center gap-1"
                    title="Edit / Transfer Enrollment"
                  >
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setWithdrawingEnrollment(st);
                    }}
                    className="p-1.5 rounded-lg border border-slate-200 text-slate-600 hover:text-rose-600 hover:border-rose-300 hover:bg-rose-50 transition-all text-xs font-semibold flex items-center gap-1"
        ...
        {withdrawingEnrollment && (
          <ConfirmDialog
            ...
            message={`Are you sure you want to withdraw ${withdrawingEnrollment.student_name || withdrawingEnrollment.full_name || 'this student'} from their current enrollment?`}
